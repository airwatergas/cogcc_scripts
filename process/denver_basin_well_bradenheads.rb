...
require mappings_directory + 'denver_basin_wells'
...
begin
  ...
  DenverBasinWells.find_by_sql("select id, well_id from denver_basin_wells order by id").each do |d|

    puts "Well: #{d.well_id}"

    BradenheadTests.find_by_sql("select distinct test_date, surface_casing_psi, end_of_test_psi, final_psi from bradenhead_tests where test_date is not null and test_date < '2015-01-01' and text_imported is true and all_psi_null is false and well_id = #{d.well_id} order by test_date").each_with_index do |b,i|
      if i == 0
