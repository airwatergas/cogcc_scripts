...
require mappings_directory + 'remediation_medias'
...
begin
  ...
  Remediations.find_by_sql("SELECT * FROM remediations WHERE in_use IS FALSE AND details_scraped IS FALSE ORDER BY id LIMIT 1").each do |remediation|
    ...
    ActiveRecord::Base.transaction do
      ...
      if impacted_media_table_length > 3 then 
        (3..max_row_num).step do |r|
          im = RemediationMedias.new
          im.remediation_id = remediation.id
      ...
      if !doc.at('td:contains("Reason for Report:")').nil? then
        ...
        if !cell_contents.split("Reason for Report:")[1].nil? then
          ...
          if !reason_report_content.split("Cause of Condition:")[0].nil? then
            remediation.report_reason = reason_report_content.split("Cause of Condition:")[0].gsub(nbsp, " ").strip
          end
        end
      end
      if !doc.at('td:contains("Cause of Condition:")').nil? then
        ...
        if !cell_contents.split("Cause of Condition:")[1].nil? then
          ...
          if !condition_cause_content.split("Potential Receptors:")[0].nil? then
            remediation.condition_cause = condition_cause_content.split("Potential Receptors:")[0].gsub(nbsp, " ").strip
          end
        end
      end
      if !doc.at('td:contains("Potential Receptors:")').nil? then
        ...
        if !cell_contents.split("Potential Receptors:")[1].nil? then
          remediation.potential_receptors = cell_contents.split("Potential Receptors:")[1].gsub(nbsp, " ").strip
        end
      end
      ...
      if !doc.at('td:contains("INITIAL ACTION:")').nil? then
        ...
        remediation.initial_action = init_action_text.gsub(nbsp, " ").strip
      end
      if !doc.at('td:contains("SOURCE REMOVED:")').nil? then
        ...
        remediation.source_removed = src_removed_text.gsub(nbsp, " ").strip
      end
      if !doc.at('td:contains("HOW REMEDIATE:")').nil? then
        ...
        remediation.how_remediate = how_rem_text.gsub(nbsp, " ").strip
      end
      if !doc.at('td:contains("MONITORING PLAN:")').nil? then
        ...
        remediation.monitoring_plan = mon_plan_text.gsub(nbsp, " ").strip
      end
      if !doc.at('td:contains("5-RECLAMATION PLAN:")').nil? then
        ...
        remediation.reclamation_plan = rec_plan_text.gsub(nbsp, " ").strip
      end
      if !doc.at('td:contains("Conditions of Approval:")').nil? then
        ...
        remediation.approval_conditions = app_cond_text.gsub(nbsp, " ").strip
      end
