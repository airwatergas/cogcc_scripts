...
# UNIX shell script to run scraper: while true; do ./scout_card_html_scrape.rb & sleep 6; done
...
require mappings_directory + 'scout_card_scrapes'
require mappings_directory + 'scout_cards'
require mappings_directory + 'sidetracks'
require mappings_directory + 'objective_formations'
require mappings_directory + 'planned_casings'
require mappings_directory + 'completed_casings'
require mappings_directory + 'completed_formations'
require mappings_directory + 'completed_intervals'
require mappings_directory + 'formation_treatments'
...
begin
  ...
  if ScoutCardScrapes.where(in_use: true).count == 0 then

#  ScoutCardScrapes.find_by_sql("SELECT * FROM scout_card_scrapes WHERE well_id <> 6709041 AND in_use IS FALSE AND html_saved IS FALSE ORDER BY well_id DESC LIMIT 1").each do |well|
  ScoutCardScrapes.find_by_sql("SELECT * FROM scout_card_scrapes WHERE well_id = 6709041").each do |well|
    ...
    ActiveRecord::Base.transaction do
      ...
      if doc.at('h3:contains("could not be found.")').nil? then
        ...
        if confidential then
          sc.completion_data_confidential = true
        end
          ...
          if contained_node.to_s == end_sidetrack_comment then
            ...
            if !obj_form_html.nil? then
              ...
              (1..num_rows).step do |row|
                of = ObjectiveFormations.new
                of.well_id = well.well_id
                of.scout_card_id = sc.id
                of.sidetrack_id = st.id
            ...
            if !plan_case_html.nil? then
              ...
              (1..num_rows).step(2) do |row|
                pc = PlannedCasings.new
                pc.well_id = well.well_id
                pc.scout_card_id = sc.id
                pc.sidetrack_id = st.id
            ...
            if !confidential then
              ...
              if !comp_case_html.nil? then
                ...
                (1..num_rows).step(2) do |row|
                  cc = CompletedCasings.new
                  cc.well_id = well.well_id
                  cc.scout_card_id = sc.id
                  cc.sidetrack_id = st.id
              ...
              if !comp_form_html.nil? then
                ...
                (1..num_rows).step do |row|
                  cf = CompletedFormations.new
                  cf.well_id = well.well_id
                  cf.scout_card_id = sc.id
                  cf.sidetrack_id = st.id
              ...
              if bore_html.at('td:contains("No additional interval records were found for sidetrack")').nil? then
                ...
                loop do
                  ...
                  if interval_contained_node.to_s == end_interval_comment then
                    ...
                    ci = CompletedIntervals.new
                    ci.well_id = well.well_id
                    ci.scout_card_id = sc.id
                    ci.sidetrack_id = st.id
                    ...
                    if !interval_html.at('td:contains("Formation Treatment")').nil? then
                      ...
                      loop do
                        ...
                        if treat_contained_node.to_s == end_treat_comment then
                          ...
                          ft = FormationTreatments.new
                          ft.well_id = well.well_id
                          ft.scout_card_id = sc.id
                          ft.sidetrack_id = st.id
                          ft.completed_interval_id = ci.id
