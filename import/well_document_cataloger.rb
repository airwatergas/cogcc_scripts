...
# COGCC Well Document Cataloger

# UNIX shell script to run scraper: while true; do ./well_document_cataloger.rb & sleep 5; done
...
require mappings_directory + 'well_documents'
...
begin
  ...
  ScrapeStatuses.find_by_sql("SELECT * FROM scrape_statuses WHERE in_use IS FALSE AND document_scrape_status = 'not scraped' LIMIT 1").each do |well|
    ...
    ActiveRecord::Base.transaction do
      ...
      if link_count > 0 then
        ...
        ActiveRecord::Base.transaction do
          ...
          well.document_scrape_status = 'scraped/found'
          well.in_use = false
          well.save!
        ...
        well.document_scrape_status = 'scraped/none'
        well.in_use = false
        well.save!
