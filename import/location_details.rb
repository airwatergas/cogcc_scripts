...
# COGCC Facility LOCATION Details Scraper

# while true; do ./location_details.rb & sleep 6; done
...
require mappings_directory + 'locations'
require mappings_directory + 'location_wells'
...
begin
  ...
  Facilities.find_by_sql("SELECT * FROM facilities WHERE facility_type_id = 13 AND in_use IS FALSE AND details_scraped IS FALSE ORDER BY id LIMIT 1").each do |f|
    ...
    ActiveRecord::Base.transaction do
      ...
      if well_check != "Sorry, no associated wells could be found." then
        ...
        (well_row_start..num_data_table_rows).step(4) do |r|
          ...
          w.facility_id = f.id
          w.location_id = d.id
          w.created_at = f.created_at
          w.updated_at = f.updated_at
