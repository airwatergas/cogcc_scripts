...
require mappings_directory + 'complaint_issues'
require mappings_directory + 'complaint_notifications'
...
begin
  ...
  Complaints.find_by_sql("SELECT * FROM complaints WHERE in_use IS FALSE AND details_scraped IS FALSE ORDER BY id LIMIT 1").each do |complaint|
    ...
    ActiveRecord::Base.transaction do
      ...
      ci = ComplaintIssues.new

      ci.complaint_id = complaint.id
      ...
      # DEBUG facility_id not being recorded - FIX on next import
      if !doc.at('td:contains("Facility ID::")').nil? then
        complaint.facility_id = doc.at('td:contains("Facility ID::")').next_element.text.gsub(nbsp, " ").strip
      end
